...
public class RUMDbSearchEngine {
	...
	{
		String newWord = "";
		ArrayList<ArrayList<String>> contents = readInputFile(inputFile);
		for(int i = 0; i < contents.size(); i++)
		{
			for(int j = 1; j < contents.get(i).size(); j++)
			{
				newWord = isWord(contents.get(i).get(j));
				if(newWord!=null)
				{
					insertWordLocation(newWord, new Location(contents.get(i).get(0), j));
				}
			}
		}
	...
	public void insertWordLocation (String word, Location loc) 
	{
		...
		{
			temp = new WordOccurrence(word);
			temp.addOccurrence(loc);
			WordOccurrence next = hashTable[hashFunction(word)];
			hashTable[hashFunction(word)] = temp;
			temp.next = next;
			if(getLoadFactor()>threshold) rehash(hashSize*2);
			wordCount++;
		}
        // COMPLETE THIS METHOD
    }
	...
	private void rehash (int newHashSize)
	{
		...
		hashSize = newHashSize; // COMPLETE THIS METHOD
       for(int i = 0; i < oldHashSize; i++)
	   {
		   WordOccurrence pointer = hashTable[i];
			while(pointer!=null)
			{
				int index = hashFunction(pointer.getWord());
				WordOccurrence temp = newHashTable[index];
				WordOccurrence newNode = new WordOccurrence(pointer.getWord());
				for(int j = 0; j < pointer.getLocations().size(); j++)
				{
					newNode.addOccurrence(pointer.getLocations().get(j));
				}
				newNode.next = temp;
	...
	public WordOccurrence getWordOccurrence (String word) {
        ...
        // COMPLETE THIS METHOD
       WordOccurrence pointer = hashTable[hashFunction(word)];
	   while(pointer != null)
	   {
	...
	public ArrayList<MovieSearchResult> createMovieSearchResult (String wordA, String wordB) 
	{
		...
		WordOccurrence wordOccA = getWordOccurrence(wordA);
		if(wordOccA == null) return null;
		WordOccurrence wordOccB = getWordOccurrence(wordB);
		if(wordOccB == null) return null;


		MovieSearchResult temp;
		for(Location a: wordOccA.getLocations())
		{
			temp = findMovieSearchResult(a.getTitle(), result);
			if(temp != null)
			{
				temp.addOccurrenceA(a.getPosition());
			}
			else if(temp == null)
			{
				MovieSearchResult movie = new MovieSearchResult(a.getTitle());
				movie.addOccurrenceA(a.getPosition());
				result.add(movie);
			}
		}
		for(Location b: wordOccB.getLocations())
		{
			temp = findMovieSearchResult(b.getTitle(), result);
			if(temp != null)
			{
				temp.addOccurrenceB(b.getPosition());
			}
			else if(temp == null)
			{
				MovieSearchResult movie = new MovieSearchResult(b.getTitle());
				movie.addOccurrenceB(b.getPosition());
				result.add(movie);
			}
		}
		return result;
	}
	private void movieSearchWordOccurrences(String word, ArrayList<MovieSearchResult> result, WordOccurrence wordOccA, int aOrB)
	{
		MovieSearchResult temp;
//		WordOccurrence wordOccA = getWordOccurrence(word);
			for(Location a: wordOccA.getLocations())
			{
				temp = findMovieSearchResult(a.getTitle(), result);
				if(temp == null)
				{
					temp = new MovieSearchResult(a.getTitle());
					temp.addOccurrenceA(a.getPosition());
				}
				if(aOrB == 0)
				{
				...
				{
					temp.addOccurrenceB(a.getPosition());
				}
			}
	}

	private MovieSearchResult findMovieSearchResult(String movieTitle, ArrayList<MovieSearchResult> results)
	{
		for(MovieSearchResult movie: results)
		{
			if(movie.getTitle().equals(movieTitle)) return movie;
		}
		return null;
	}
	...
	public void calculateMinDistance(MovieSearchResult msr)
	{
		...
		int min;
		int temp;
		if(locationsA.isEmpty() || locationsB.isEmpty()) {msr.setMinDistance(-1); return;}
		int pointerA = 0;
		int pointerB = 0;
		min = Math.abs(locationsA.get(pointerA)-locationsB.get(pointerB));
		while(true)
		{
			if(locationsA.get(pointerA) < locationsB.get(pointerB))
			{
				pointerA++;
			}
			else
			...
			if(pointerA >= locationsA.size() || pointerB >= locationsB.size())
			{
				msr.setMinDistance(min);
				return;
			}
			temp = Math.abs(locationsA.get(pointerA)-locationsB.get(pointerB));
	...
	public ArrayList<MovieSearchResult> topTenSearch(String wordA, String wordB)
	{
		...
		{
			MovieSearchResult movie = results.get(i);
			calculateMinDistance(movie);
			if(movie.getMinDistance() == -1)
			{
				results.remove(i);
				i--;
			}
		...
		ArrayList<MovieSearchResult> finalResults = new ArrayList<MovieSearchResult>();
		for(int i = 0; i <=9; i++)
		{
